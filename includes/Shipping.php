...
class Shipping {
	...
	public function MaxWeight( $id_shipping_method ) {

		$res = 0;

		$dalc = new DALC();

		$shipping_method = $dalc->SQL_SelectItem('shipping_methods', NULL, $id_shipping_method);

		$res = $shipping_method['max_weight'];
	...
	public function Calc_RussianPostFirstClass( $order_items ) {
		...
		$res += 114.46 + (ceil($weight / 100) - 1) * 17.11;
