<?php
	...
	foreach ( $data['cart_items'] as $item ) {
		...
		$data['weight'] += 2050 * $item['qty'];

	}

	$data['weight_str'] = number_format( $data['weight'] / 1000, 2, '.', '' );
	...
	if ( $ems_city && $data['weight_str'] ) {
		...
		if (isset($data_json)) {
			...
			if ( $data_json['rsp']['stat'] == 'fail' ) {
				...
				$data['shipping_cost'] = $data_json['rsp']['price'];
				...
				$data['shipping_cost_str'] = $data['shipping_cost'] . ' руб.';
