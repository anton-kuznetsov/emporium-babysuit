...
class CreateOrderPage_BLLC {
	...
	public function NewOrder( $data ) {
		...
		// Чистка строки параметров для расчета доставки

		$data['shipping_params'] = str_replace('\"', '"', $data['shipping_params']);
		...
		if ( $cart['id_order'] == 0 ) {
			...
			$order['shipping_cost']      = Shipping::Calc(
				$data['id_shipping_method'],
				$data['shipping_params'],
				$cart_items
			);
			...
			$order_dalc->SQL_UpdateItems(
				'orders',
				array( $order ),
				array( 'dt', 'id_shipping_method', 'shipping_params', 'shipping_cost' )
			);
	...
	public function SendToClient( $id_order ) {
		...
		$order = $create_order_bllc->GetData( $id_order );
		...
		$message .=
				...
				<p>
					<b>Доставка</b><br />
					Способ: ' . $order['shipping_method']['label'] . '<br />
					Адрес: ' . $order['country']['label'] . ', ' . $order['country_region']['label'] . '<br />' . $order['shipping_address'] . '
				</p>
				<p>
					<b>Стоимость</b><br />
					Товар: ' . $order['products_cost_str'] . '<br />
					Доставка: ' . $order['shipping_cost_str'] . '<br />
					<b>Сумма заказа: ' . $order['total_str'] . '</b>
				</p>
				<p>
					<b>Менеджер по работе с клиентами</b><br />
					Имя: ' . $order['client_manager']['fio'] . '<br />
					Телефон: ' . $order['client_manager']['phone'] . '
				</p>
			</body></html>';
...
// 		$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
		...
		// Дополнительные заголовки
		$headers .= 'To: <' . $order['client_manager']['email'] . '>' . "\r\n";
		$headers .= 'From: BabySuit.Ru <no-reply@babysuit.ru>' . "\r\n";
		$headers .= 'Cc: ' . "\r\n";
		$headers .= 'Bcc: ' . "\r\n";
